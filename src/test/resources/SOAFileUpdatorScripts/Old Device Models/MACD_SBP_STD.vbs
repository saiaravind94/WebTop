...
' New outpul file will be generated with MACD_SBP_STD_<UpdatedOppornityID>.xlsm pattern
...
Else
	...
	With objWorkbook.Worksheets(curSheetName)
		...
		newFileName = SOAFolderPath & "\MACD_SBP_STD_" & (SFOpportunityId + 1) & ".xlsm"
		
		commonFieldUpdates objWorkbook, curSheetName, "MACD_SBP_STD_"
		
		'---------------------------------------------------------------------------------------------------------------------
		.Cells(getRowIdx(curSheetName, "Unified Communication Seats"), columnIdxToUpdate).Value = 5
		.Cells(getRowIdx(curSheetName, "Polycom SoundPoint VVX 310 HD"), columnIdxToUpdate).Value = 5
		'---------------------------------------------------------------------------------------------------------------------
