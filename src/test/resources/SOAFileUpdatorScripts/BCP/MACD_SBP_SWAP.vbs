...
' New outpul file will be generated with OneDECTBase8HandSet3Repeater_<UpdatedOppornityID>.xlsm pattern
...
regEx.Pattern = "OneBS8DECTBase3Repeater_.*.xlsm"
...
Else
	...
	With objWorkbook.Worksheets(curSheetName)
		...
		SFOpportunityId = .Cells(getRowIdx(curSheetName, "SF Opportunity ID:"),getColumnIdx(curSheetName, "SF Opportunity ID:")+1).Value
		
		newFileName = SOAFolderPath & "\OneBS8DECTBase3Repeater_" & (SFOpportunityId + 1) & ".xlsm"
		
		BCPSOAUpdate objWorkbook, curSheetName, "OneBS8DECTBase3Repeater_"
		
		' ----------------------------------------------------------------------------------------------------------------------
		.Cells(getRowIdx(curSheetName, "Unified Communication Seats"), getColumnIdx(curSheetName, "Unified Communication Seats")+1).Value = 8
		.Cells(getRowIdx(curSheetName, "Panasonic TGP600 Smart IP Cordless Phone Base Station"), getColumnIdx(curSheetName, "Panasonic TGP600 Smart IP Cordless Phone Base Station")+1).Value = 1
		.Cells(getRowIdx(curSheetName, "Panasonic KX-TPA65 Desktop DECT Phone"), getColumnIdx(curSheetName, "Panasonic KX-TPA65 Desktop DECT Phone")+1).Value = 8
		.Cells(getRowIdx(curSheetName, "Panasonic KX-A406 Cordless Repeater"), getColumnIdx(curSheetName, "Panasonic KX-A406 Cordless Repeater")+1).Value = 3
		'---------------------------------------------------------------------------------------------------------------------
