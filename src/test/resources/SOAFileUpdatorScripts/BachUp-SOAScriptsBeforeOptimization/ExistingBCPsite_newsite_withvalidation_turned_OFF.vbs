...
' New outpul file will be generated with BCP_PERNR_Validation_<UpdatedOppornityID>.xlsm pattern
...
Else
	...
	With objWorkbook.Worksheets(curSheetName)
		...
		newFileName = SOAFolderPath & "\BCP_PERNR_Validation_" & (SFOpportunityId + 1) & ".xlsm"
		
		BCPSOACommonFieldUpdate_V1_44 objWorkbook, curSheetName, "BCP_PERNR_Validation_", mailIdList
		
		'*****************************************************************************************************
		.Cells(getRowIdx(curSheetName, "Site Type* :"), columnIdxToUpdate).Value = "Standard"
		.Cells(getRowIdx(curSheetName, "Sales Rep PERNR:"), 2).value = Right(getTimestamp(), 8)
		'*****************************************************************************************************
