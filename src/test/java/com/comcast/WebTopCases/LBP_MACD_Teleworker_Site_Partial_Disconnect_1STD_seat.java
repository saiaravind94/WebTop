...
public class LBP_MACD_Teleworker_Site_Partial_Disconnect_1STD_seat
	...
	public void verifyGroupServicesBeforeMACD() throws Exception {		
		...
		try {
			...
			broadsoftGroupPage.verifyGroupServices("TWBVEStandardSeatv2.1", 2, 2, 2);
			broadsoftGroupPage.verifyGroupServices("TWBVEUnifiedSeatw/vmail2.1", 2, 2, 2);
		} catch (Exception Ex) {
	...
	public void terminateStdSeat() throws Exception {
		...
		try {
			...
			for(int idx = 0; idx<dataDump.getValue("StdANIList_before").split("\\|").length;idx++)
			{
				if(!BTNNo.equalsIgnoreCase(dataDump.getValue("StdANIList_before").split("\\|")[idx])){
					dataDump.setValue("ANINumberToTerminate_RT", dataDump.getValue("StdANIList_before").split("\\|")[idx]);
					stdSeatIdxToTerminate = idx;
					break;
				}				
			}
			accountProfileTab.terminateSeat("standard", stdSeatIdxToTerminate + 1);			
		} catch (Exception Ex) {
			report.reportHardFailEvent("Terminate Seat", "Terminate seat Failed!!!", Status.FAIL);
			String eMsg = "Error in Terminating seat --- " + Ex.getMessage();
			log.error(eMsg);
	...
	public void verifyGroupServicesAfterMACD() throws Exception {		
		...
		try {
			...
			broadsoftGroupPage.searchGroups("Group ID", "Equal To", dataDump.getValue("GroupID"));
			broadsoftGroupPage.verifyGroupServices("TWBVEStandardSeatv2.1", 1, 1, 1);
			broadsoftGroupPage.verifyGroupServices("TWBVEUnifiedSeatw/vmail2.1", 2, 2, 2);
		} catch (Exception Ex) {
			report.reportHardFailEvent("Broadsoft Standard/UC Seat verification", "Broadsoft Standard/UC Seat verification failed", Status.FAIL);
			String eMsg = "Error in Broadsoft Standard/UC Seat verification failed" + Ex.getMessage();
